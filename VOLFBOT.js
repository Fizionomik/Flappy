...
vk.updates.use(async (message, next) => {  
	...
	✂ ➾ КНБ [камень/ножницы/бумага] [ставка] - Играть в КНБ
	...
	vk.updates.hear(/^(?:Меню|начать)$/i,  (message) => { 
	let user = acc.users[user_id(message.user)];
...
◖◗🔧 Настройки 🔧◖◗ - Настройки
 👤Бот👤 - Инфа о версии бота
 Помощь - инфа вобщем

    `)
   });	
   
	vk.updates.hear(/^(?:Передача другому игроку)$/i,  (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance == null) return message.send(`⚠ Произошла ошибка ! Пожалуйста, сообщите в репорт.`);
	return message.send(`
 ═ ► ═ ► ═ ► ═ ► ═ ► ═ ► ═ ► ═ ► ═ ► ═ ► ═ ► ═ ►
...
🤝● Перевести [ID] [сумма] - Перевести деньги в банк игроку
	...
	vk.updates.hear(/^(?:Заработок)$/i,  (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance == null) return message.send(`⚠ Произошла ошибка ! Пожалуйста, сообщите в репорт.`);
	return message.send(`
...
🎁 Для Вас доступны подарки ! Чтобы открыть, напиши " подарок "
 ...
 vk.updates.hear(/^(?:Состав)$/i,  (message) => {
    ...
    ║ Дизайнер - Ярослав Новиченок
    ║ Тестер - Федя Петкевич
    ║ ....
...
🤙🏻 Посмотреть счет всех банд, напиши " fcapt "
   ...
   vk.updates.hear(/^(?:помощь)$/i,  (message) => { 
	...
	return message.send(`
Это раздел помощь.
Здесь вы получите всю информацию об игре.


Бот Gliger был создан 23.07.2019 создателем [id196981361|Костей Баушевым]. Наш бот был создан чисто в развлекательных целях. Всю информацию о созданании можно получить у создателя.
...
😏 Акция действует до 20 августа 2019 года !
	...
	if(message.$match[3]){
		...
		vk.api.call("utils.resolveScreenName", {
			...
			return message.send(`
				Игрок: ${acc.users[id].prefix}
				ID: ${id}
				Статус: ${acc.users[id].level.toString().replace(/0/gi, "Игрок").replace(/1/gi, "VIP").replace(/2/gi, "Модератор").replace(/3/gi, "Администратор").Feplace(/4/gi, "Гл.Администратор").replace(/5/gi, "Создатель")}
				`);
		})
		return;
	}
...
vk.updates.hear(/^(?:setmoney)\s?([0-9]+)?/i, message => {
	...
	if(user.level == 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 300000) return message.send(`💰 ➾ Пример: 'setmoney [1-300000]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level == 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 1000000) return message.send(`💰 ➾ Пример: 'setmoney [1-1000000]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level > 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 8000000) return message.send(`💰 ➾ Пример: 'setmoney [1-8000000]'`);
		user.balance += Number(message.$match[1]);
	}
...
//////////////////////////////////////
	...
	///////////////////////////////
	vk.updates.hear(/^(?:фермвв)$/i, (message) => {
		let dev = '';   
		return message.send(`
			👉 ➾ МАЙНИНГ ферма:
			♻ Ферма " теплые яйца "
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	vk.updates.hear(/^(?:казино)\s?([^\s  ].*)?/i, (message) => {
        ...
        let mnojitel = [1,2].random();
        let win = ['🌚|🌚|🌚','🔸|🔸|🔸','🎲|🎲|🎲'].random();
        let lose = ['🌚|🎉|🔸','🔸|🎉|🔸','🎲|🎉|🎲'].random();

        if(rand(1,100,1000) < 30){
        	let balance = amount;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	///////////////////////////////////////////////////НОЖНИЦЫ////////////////////
	vk.updates.hear(/^(?:КНБ)?\s([^\s].*)?\s(.*)/i, message => {
		if(!message.$match[1] || !message.$match[2]) return message.send(`🔸 ➾ Пример команды: КНБ [камень/ножницы/бумага] [ставка]`)
		let amount = parserInt(message.$match[2]);   
		let user = acc.users[user_id(message.user)]; 
		let id = user_id(message.user) 
		if (amount > acc.users[id].balance || amount < 1) return message.send(`🎉 ➾  Ставка не может превышать баланс или быть ниже 1$`);
		...
		if(!Number(amount)) return message.send(`🔸 ➾ Ставка должна быть числом!`); 
		 	...
		 	if(message.$match[1] == 'камень'){
				if(rand(1,2) == 1){ 
					...
					game_log(user_id(message.user), 'КНБ', amount, 0)
					user.game.binlose += 1;
					user.balance -= amount;
	...
	vk.updates.hear(/^(?:ставкаотклк)\s?([^]+)?\s([^\s	].*)/i,  message => {
...
////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////САПЕР///////////////////////////////////////////
    vk.updates.hear(/^(?:Алок)\s?([0-9]+)?\s([^\s	].*)/i, message => {
		let i = parserInt(message.$match[2]); 
		let user = acc.users[user_id(message.user)];
...
//////////////////////////////////////////////////////// промики
 ...
 vk.updates.hear(/^(?:промокод)\s?([^]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:создать)\s?([0-9]+)?\s?([^\s	].*)?\s?([^\s	].*)?/i, async (message, bot) => {
...
//////////////////////////////////////
 ...
 vk.updates.hear(/^(?:Фермы)\s?([1-9]+)?/i, (message) => {
 	let user = acc.users[user_id(message.user)];  
		...
		if(!message.$match[1]){
			...
			💸 ➾ Для аренды фермы на час: Фермы [номер]
...
vk.updates.hear(/^(?:риск)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance < 50000) return message.send(`💣 ➾ У вас не хватает 500.000 !$`);
	let rez = [true, false].random();
	if(rez == false){
		let text = [].random(); 
		user.balance -= 50000;
...
vk.updates.hear(/^(?:restart)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.level < 5) return message.send(`Вы не гл. Администратор`);
